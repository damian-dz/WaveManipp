...
{
    int step = m_waveProperties.getNumChannels();
    int resSize = m_numSamples / step / binSize;
    int remainder = (m_numSamples / step) % binSize;
    if ((m_numSamples / step) % binSize != 0) {
        ++resSize;
    }
    ...
    for (int i = 0; i < resSize; ++i) {
        ...
        for (int j = channel; j < endPoint; j += step) {
